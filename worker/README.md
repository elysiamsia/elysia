...
**一次改动能同时解决 11.5 和 11.7 两个问题：换成 Durable Objects。** DO 是单线程执行的，天然支持原子递增；而且它按**请求数**计费（每天 10 万次），不受 KV 这个 1000 次写入的卡脖子限制。代价是 Worker 重写约 60 行。

**尚未决定是否要换。** 判断依据：如果生日那天的献花数会接近或超过 500，就该换。
