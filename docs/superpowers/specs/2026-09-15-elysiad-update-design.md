...
#### 实施后的两处修正（本节以下内容与代码不符，以这里为准）

| 项 | 本节原文 | 实际实现 | 为什么 |
|---|---|---|---|
| 接口地址 | 域名 `flowers.elysiad.top`（跨域） | **同源**：`elysiad.top/count`、`elysiad.top/flower`；`flowers.elysiad.top` 只留给本地开发 | 2026-09-16 需求方实拍：QQ 浏览器上跨域请求被拦 → 点献花静默降级成「本机累计 N 朵」，桌面浏览器同一时刻显示共享计数。与 §6.7 留言簿同一个坑，献花当时漏改了。详见 `worker/README.md` §13 |
| 超时 | 1.5 秒 | 载入 4000ms / 点击 3000ms，两者分开 | 大陆冷启动到 Cloudflare 的 TLS 握手实测可到 1.66 秒，1500ms 会把**健康**的接口误判成故障——访客看到「本机累计 1 朵」，以为自己把计数搞坏了 |

改同源之后握手那一步省掉了（同一条连接），但超时值**故意不再收紧**：收紧只会重新引入第二个坑。
...
#### 结论：QQ / 微信始终不出预览，接受现状（2026-09-16）

需求方实测 QQ 与微信均不显示预览卡片，**换用新链接触发重新抓取后仍然如此**。需求方决定不再追。

**已排除的原因**（逐项实测，都有数据）：

| 假设 | 实测结果 | 结论 |
|---|---|---|
| 标签缺失或写错 | 第三方 OG 解析服务（Microlink）把 `title` / `description` / `image.url` / `image.size` / `logo` 全部正确读出 | ❌ 排除 |
| 爬虫抓不到页面 | 三种爬虫 UA（微信 / QQ / bytespider）访问均返回 200 + 完整标签；robots 未拦截 | ❌ 排除 |
| 图片用了 WebP | 是 PNG，后改为 JPEG | ❌ 排除 |
| `og:image` 是相对地址 | 是绝对地址 `https://elysiad.top/images/og-1.jpg` | ❌ 排除 |
| 平台缓存了"无预览"的旧结果 | 换新链接（`/index.html`）后仍然不出 | ❌ 排除 |
| 图片太大（537 KB）导致爬虫超时 | 已降到 89 KB | ❌ 排除（但值得做，见下） |
| 页面抓取太慢（5.26 秒）导致爬虫超时 | 已降到 0.63 秒 | ❌ 排除（但值得做，见下） |

**排除到最后，最可能的原因是腾讯自家的域名策略**——`.top` 是廉价后缀、国内滥用的不少，腾讯可能对这类域名的外链不做卡片渲染。**这一条无法验证，也无法绕过**（除非换域名，代价过大）。

**顺带拿到的两项收益**（这才是这轮排查的真正价值，且与分享预览无关）：

| | 改前 | 改后 |
|---|---|---|
| og 图体积 | 537~630 KB | 89~129 KB |
| 腾讯爬虫抓页面 | 5.26 s | 0.63 s |
| 缓存命中时的响应 | 0.566 s（每次回源） | **0.181 s** |

第三条是**全站**的速度——根因是 **Cloudflare 默认不缓存 HTML**（`cf-cache-status: DYNAMIC`，即使源站给了 `Cache-Control: max-age=600`）。加一条 Cache Rule（`Hostname 等于 elysiad.top` → 符合缓存条件 → 边缘 TTL 2 分钟）之后，图片与 JS 也从"每次回源"变成"边缘直接出"。

> ⚠ 加 Cache Rule 时必须同时给 API 响应标 `Cache-Control: no-store`，否则 `/notes` 会被边缘缓存 2 分钟——
> 后果是「刚通过的留言，别人两分钟后才看得到」。已由 `worker/src/index.js` 的 `json()` 统一加上。
