...
> 需求方 2026-09-17 定的标准：**每位英桀至少 3 个彩蛋**。
> 查过 kalpas / su / villv **都用了「点名字」那一套**，所以樱**不点名字**。
...
3. **「等待」**（全站独一份的**闲置彩蛋**）
   往下滚过开场之后**手不动待够 25 秒**，会有一瓣樱从屏幕顶上慢慢落下来，
   然后她那句「等待，曾经那是我用以谋生最重要的技巧，却不再是我现在的目的了。」
   才浮出来 —— 这次不是打给你看的，是**等到了你**之后才说的。

   > ⚠ 两个细节是实测才发现的：
   > · **还停在开场那一屏时不启动计时** —— 否则开场打字机刚打完同一句话，
   >   等一会儿又原样浮出一遍，看着像 bug（第一版截图就是这样）。
   >   现在只在 `scrollY > innerHeight * 0.55` 之后才起算。
   > · 那句话会浮在任何一段内容上面，所以给了它一层**磨砂底**，
   >   不然压着时间轴卡片看不清。
   >
   > 减动偏好下**不放花瓣**（不动的图也算动效），只让那句话浮出来。
...
### 4.7 给三个「空页面」补齐彩蛋：阿波尼亚 / 伊甸 / 凯文（2026-09-17）

**起因**：需求方要求每位英桀至少 3 个彩蛋。逐页数过之后发现这三页
**整页一个都没有** —— 每页只有 1 个共享的点击涟漪监听器，
阿波尼亚那页**连 `showToast` 函数都不存在**。

| 页 | 新增的专属模块 | 三个彩蛋 |
|---|---|---|
| **aponía** | 「她的丝」—— 三圈命运丝绕着一个 ✧，缓慢反向自转 | ① ★**连点 6 次 → 她施「戒律」**（丝线从四周收进来缚住你）② 点丝环换台词 ③ 滚到结尾，九缕丝垂下 +「我憎恨命运…我感激命运…」 |
| **eden** | 「她的歌」—— 黑胶唱片 + 唱臂，**点了真的发声**（三个振荡器合成 C 大调和弦，不加载音频文件） | ① 唱片 ② ★**双击 → 举杯祝酒**（34 道金光升起 + 琥珀遮罩）③ 滚到结尾，一颗星陨落 +「最璀璨的星光…」 |
| **kevin** | 「他的剑」—— 一柄**封在冰里**的剑 | ① ★**点够 5 次才断**（点一次裂一道，第一次裂时说那句「只有我能亲手折断它」）② 点开场 ❄ → 冰鸟飞过 +「鸟儿为什么会飞？」③ 滚到结尾，冰面浮出那句承诺 |

**三位各有一个「独一份」的机制**：连点（aponía）/ 双击（eden）/ 需要够多次才生效（kevin）。
其余彩蛋的**台词全部取自各自 `quotes` 池**（本来就都有出处），没有一句是编的。

#### ⚠ 一个 bug：**从别的页抄函数名会直接报错**

凯文那页我照樱页写了 `petalBurst(...)` —— 可**凯文页根本没有这个函数**
（它只有 `flakes` / `embers` / `stars`，没有迸发工具）。
调用时就是 `ReferenceError`。

> 📌 **教训**：这些页是同一个模板**各自复制**出来的，但**共享层只抽走了
> `ElysiaShared` 那几个**（P1 Task 1–9 抽的那些）。页面内部的私有函数
> （粒子数组、迸发、工具）**每页一套，名字不一样甚至没有**。
> **跨页抄代码前，先 grep 那个函数在目标页存不存在。**
>
> 已给凯文补了个 `shardBurst()`，用他自己的冰晶视觉。

#### 验收

三页各自：功能实测通过（模块 / 三个彩蛋逐条验）、
快照比对**差异全部落在该页、全部是 height**（各新增一节），其余页零差异。
`check_aria_labels` 8 页、`check_reduced_motion` 5 页双向，全绿。
...
| **补齐老页面的彩蛋** | ⭐ 见下方盘点 | **与新页面并行** |
| 10 index 专项 | ❌ 首页专属 | 新页面之后 |
| 11 armor 专项 | ❌ 异形页专属 | 新页面之后 |
| 12 收尾清理 | ➖ 好看 | 最后 |

##### ⚠ 彩蛋盘点：**三个页面几乎是空的**（2026-09-17 实测）

需求方要求**每位英桀至少 3 个彩蛋**。逐页数过之后：

| 页 | 彩蛋 | 说明 |
|---|---|---|
| kalpas | **3** | 面具裂纹 / 点名字挨骂 / 结尾的心声 |
| villv | **4** | 点名字切人格 / 抽卡 / 结尾返场 / 对凯文武装666 |
| sakura | **3** | 鞘中刀 / 结尾的勿忘我 / 「等待」 |
| su | 2～3 | 点名字得禅语 / 长按娑之凝视 / 结尾心声 |
| **aponia** | **3** ✅ | 连点施「戒律」/ 命运之丝 / 结尾垂丝——见 §4.7 |
| **eden** | **3** ✅ | 唱片 / 双击举杯 / 陨落的星——见 §4.7 |
| **kevin** | **3** ✅ | 折断的剑 / 冰鸟 / 结尾的承诺——见 §4.7 |
| index | 有 | 点标题 5 次→水晶花雨（**台词池为空，现在不触发**）/ 生日倒计时 / 献花 / 明信片 |

> ✅ **三页的欠账已于 2026-09-17 补齐**（aponia / eden / kevin 各 3 个），见 §4.7。
> 现在**每一位英桀都至少有 3 个彩蛋**了。
>
> 📌 以后新建页面时，**彩蛋要一并在第一版里做掉** —— 别先上线再补，
> 这三页就是因为先上线了、又没人回来看，空了很久。

> 💡 **补彩蛋的两条经验**（§4.7 三页实测出来的）：
> ① 素材直接取该页自己 `quotes` 区的台词（本来就都有出处）；
> ② **别从别的页抄函数名** —— 那些页是同一模板各自复制出来的，
>    私有函数每页一套、名字不同甚至没有（凯文那次就这么踩了）。
>    抄之前先 grep。
