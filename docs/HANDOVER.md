...
| **新建英桀页** | 🟡 **1 / 6 完成**：樱 `/sakura/` 已上线，见 §4.6 |
...
### 4.6 第一位新英桀页：樱（2026-09-17）

**`/sakura/` —— 逐火十三英桀第 Ⅷ 位 ·「刹那」樱。**
这是「13 页大计」里新建的第一页，也是后面 5 页的参照。

#### 怎么建（做法本身值得照抄）

**做法：把 `su/index.html` 复制成骨架，再用三个脚本逐块替换。**
每处替换都断言「恰好命中 N 次」，命中数不对就**整体中止、不写盘** ——
这样不会留下改一半的页面。（脚本是临时的，没进仓库。）

比「从头写一个」稳得多：共享层接线、响应式、减动、观察器全都是现成且已被验证的。

#### 这一页「自己的特色」（需求方要求：每位英桀不重样）

| 维度 | 樱 | 与别人的区别 |
|---|---|---|
| **配色「夜樱」** | 冷靛黑 + 樱粉 + 冰蓝 | index/villv 是**暖**紫粉、kevin 是中性冷蓝、su 是青绿。樱是**冷靛底上的夜樱粉**，冰蓝只作细细一道 |
| **画布特效** | `sakuraCanvas` 落樱 + 冰尘 + **偶发的「刹那」刀光** | index 只有花瓣；樱多一层「静止中突然的一瞬」—— 一道极细的冰蓝斜光，约 7~15 秒一次 |
| **专属模块** | **「鞘中刀」** | su 木鱼、kalpas 怒气条、villv 抽卡 —— 樱是**一柄不肯出鞘的刀** |

#### 彩蛋：两段，且**是连着的**

> 需求方明确要求「每个角色的彩蛋不重样」。查过 kalpas / su / villv
> **都用了「点名字」那一套**，所以樱**故意不点名字**。

1. **鞘中刀**（全站独一份的「拒绝式」彩蛋）
   点刀 → 她**拒绝**（「…很抱歉，它决不能就这样轻易出鞘。对我来说，这是『戒律』的一部分。」）
   第二次 → 她只是看着你，鞘口的光亮了一下
   **第三次 → 她没有拔刀，而是给了你一朵「勿忘我」**（「请收下它吧，温柔的女孩。」）
   花会留在页面上，并用 `localStorage` 记住。

2. **结尾的勿忘我**（全站独一份的**跨彩蛋解锁**）
   只有真的收下过那朵花，结尾才会多开一朵花、多留一句
   ——「她一直是个怕寂寞的孩子……真希望她迎来的，是一个平静安稳的结局啊。」

> ⚠ **两句台词都有出处**（官方「圣芙蕾雅档案馆」事件-樱），不是我编的。
> 彩蛋机制可以自己设计，**话不能编** —— 这是这个项目的底线。
> 语录 10 条、时间轴 8 节、档案全部取自 `materials/樱/text_materials.md`。

#### ⚠ 两条被需求方更正过的字段（**别照材料包改回去**）

`materials/樱/` 那份材料包里，有两条与**需求方 2026-09-17 的口头更正相反**。
材料包不是错的来源，只是它按「前文明樱在乐土里是 NPC」来写的；需求方另有依据。
**以需求方的为准，别再"修"回去。**

| 字段 | 材料包写的 | **需求方确认的** |
|---|---|---|
| 生日 | 【待考】官方未公布 | **7月22日** |
| 装甲 / 可操作性 | 【待考】她并非可操作女武神，是乐土 NPC | **可操控，装甲「御神装 · 勿忘」** |

> 需求方原话：「我确认这两条就是前文明樱的，**逐火之蛾剧情里操控的就是御神装·勿忘**。」

**旁证**：材料包给出的武器「超限『御灵刀 · 寒狱冰天』」，正是御神装·勿忘的武器 ——
所以武器那条本来就与需求方的说法**对得上**，只是装甲/可操作性那栏当初判成了 NPC。

> 📌 早先我在这一页写过「非现文明八重樱」的出处标注，以及「前文明 NPC，官方未公布」。
> **出处标注保留**（这一页讲的确是前文明那位），**NPC / 非可操作的说法已删除**。

#### ⚠ 顺带发现一个**既有 bug**（尚未修，等需求方定）

`su/index.html` 的月亮装饰 `.opening-moon` **偏在屏幕最左边**（实测中心 x=239，视口中心 640）。
根因：`.opening-content` **全站都没有 CSS 规则**，它只是个普通 block，
宽度被最宽的子元素撑到 860px；块级的圆形装饰在里面就贴左了。

villv / aponia 用的是**文字**装饰（靠 `text-align:center` 天然居中），所以只有 su 中招。
樱这一页已用 `margin:0 auto` 修好；**su 的没动**（改它会改变既有页面的视觉，
要单独验证 + 单独 commit，交需求方定）。
...
# 2) 快照 + 比对。建下一张英桀页用 baseline-sakura 当参照
PYTHONIOENCODING=utf-8 python tools/snapshot.py after-<本步>
PYTHONIOENCODING=utf-8 python tools/snapshot_diff.py baseline-sakura after-<本步>; echo "退出码 $?"
...
| `screenshots/snap/baseline-seo` | **英桀名片入口 + SEO 之后** | 已用过，保留备查 |
| `screenshots/snap/baseline-sakura` | **樱页之后**（2026-09-17） | **建下一张英桀页时的参照** |
...
> 💡 **最后两条是最容易忘的**（樱那一轮实测）：**工具不会自己知道多了一个页面。**
> 手工补上这两处之后，`check_aria_labels.py` 的覆盖才从 7 页变成 8 页。

> 💡 **推荐做法：把 `su/index.html` 复制成骨架，再用脚本逐块替换。**
> 共享层接线、响应式、减动、观察器全是现成且已被验证的，比从头写稳得多。
> 樱页的具体做法见 §4.6。
...
- **`su/index.html` 的月亮装饰偏在屏幕最左边**（2026-09-17 发现，**未修**）：
  实测中心 x=239，而视口中心是 640。根因是 `.opening-content` 全站没有 CSS 规则，
